...
// This bot controls mixing fans 1 - 7
...
volatile unsigned long mF_PulseCount[7]
	{
		0, 0, 0, 0, 0, 0, 0
	};
	
unsigned long mF_millisecondsUntilDone[7]
	{
		0, 0, 0, 0, 0, 0, 0
	};
	
String mF_CommandID[7]
	{
		"", "", "", "", "", "", ""
	};
...
const int mF_RelayPin[7]
	{
		17, 18, 19, 21, 22, 23, 4
	};

const int mF_PWMChannel[7]
	{
		0, 1, 2, 3, 4, 5, 6
	};
const int mF_PWMPin[7]
	{
		16, 12, 37, 14, 13, 15, 26
	};
const int mF_RPMPin[7]
	{
		5, 36, 39, 34, 35, 32, 33
	};
...
void IRAM_ATTR rpm_fan_3() {
	if (flgCountingPulses) {
		mF_PulseCount[2]++;
	}
}

void IRAM_ATTR rpm_fan_4() {
	if (flgCountingPulses) {
		mF_PulseCount[3]++;
	}
}

void IRAM_ATTR rpm_fan_5() {
	if (flgCountingPulses) {
		mF_PulseCount[4]++;
	}
}

void IRAM_ATTR rpm_fan_6() {
	if (flgCountingPulses) {
		mF_PulseCount[5]++;
	}
}
void IRAM_ATTR rpm_fan_7() {
	if (flgCountingPulses) {
		mF_PulseCount[6]++;
	}
}
...
void setup(void) {
	...
	attachInterrupt(digitalPinToInterrupt(39), rpm_fan_3, FALLING);
	attachInterrupt(digitalPinToInterrupt(34), rpm_fan_4, FALLING);
	//attachInterrupt(digitalPinToInterrupt(35), rpm_fan_5, FALLING);
	attachInterrupt(digitalPinToInterrupt(32), rpm_fan_6, FALLING);
	attachInterrupt(digitalPinToInterrupt(33), rpm_fan_7, FALLING);
...
void callback(char* topic, byte* payload, unsigned int length){
	...
	if (strcmp(topic, "mixingFan/MF1") == 0){
		char* strtokIndx;
	...
	if (strcmp(topic, "mixingFan/MF2") == 0){
		char* strtokIndx;
	...
	if (strcmp(topic, "mixingFan/MF3") == 0 ){
		char* strtokIndx;

		strtokIndx = strtok(payloadStr,":");
		mF_PWMSpeed[2] = atoi(strtokIndx);

		strtokIndx = strtok(NULL,":");
		mF_OverSpeed[2] = atoi(strtokIndx);

		strtokIndx = strtok(NULL,":");
		mF_millisecondsUntilDone[2] = millis() + (atol(strtokIndx) * 60 * 1000);

		strtokIndx = strtok(NULL,":");
		mF_CommandID[2] = strtokIndx;

		String postCmd = "{\"RemoteHostname\":\"" + hostname + "\",\"CommandID\":\"" + mF_CommandID[2].substring(0,36) + "\"}";

		client.publish(MQTT_TOPIC_ACK, postCmd.c_str());
		flgMfRunning[2] = true;
		ledcWrite(mF_PWMChannel[2], mF_PWMSpeed[2]);
		digitalWrite(mF_RelayPin[2], LOW);
	}

	if (strcmp(topic, "mixingFan/MF4") == 0 ){
		char* strtokIndx;

		strtokIndx = strtok(payloadStr,":");
		mF_PWMSpeed[3] = atoi(strtokIndx);

		strtokIndx = strtok(NULL,":");
		mF_OverSpeed[3] = atoi(strtokIndx);

		strtokIndx = strtok(NULL,":");
		mF_millisecondsUntilDone[3] = millis() + (atol(strtokIndx) * 60 * 1000);

		strtokIndx = strtok(NULL,":");
		mF_CommandID[3] = strtokIndx;

		String postCmd = "{\"RemoteHostname\":\"" + hostname + "\",\"CommandID\":\"" + mF_CommandID[3].substring(0,36) + "\"}";

		client.publish(MQTT_TOPIC_ACK, postCmd.c_str());
		flgMfRunning[3] = true;
		ledcWrite(mF_PWMChannel[3], mF_PWMSpeed[3]);
		digitalWrite(mF_RelayPin[3], LOW);
	}

	if (strcmp(topic, "mixingFan/MF5") == 0 ){
		char* strtokIndx;

		strtokIndx = strtok(payloadStr,":");
		mF_PWMSpeed[4] = atoi(strtokIndx);

		strtokIndx = strtok(NULL,":");
		mF_OverSpeed[4] = atoi(strtokIndx);

		strtokIndx = strtok(NULL,":");
		mF_millisecondsUntilDone[4] = millis() + (atol(strtokIndx) * 60 * 1000);

		strtokIndx = strtok(NULL,":");
		mF_CommandID[4] = strtokIndx;
		
		String postCmd = "{\"RemoteHostname\":\"" + hostname + "\",\"CommandID\":\"" + mF_CommandID[4].substring(0,36) + "\"}";

		client.publish(MQTT_TOPIC_ACK, postCmd.c_str());
		flgMfRunning[4] = true;
		ledcWrite(mF_PWMChannel[4], mF_PWMSpeed[4]);
		digitalWrite(mF_RelayPin[4], LOW);
	}

	if (strcmp(topic, "mixingFan/MF6") == 0 ){
		char* strtokIndx;
		
		strtokIndx = strtok(payloadStr,":");
		mF_PWMSpeed[5] = atoi(strtokIndx);
		
		strtokIndx = strtok(NULL,":");
		mF_OverSpeed[5] = atoi(strtokIndx);
		
		strtokIndx = strtok(NULL,":");
		mF_millisecondsUntilDone[5] = millis() + (atol(strtokIndx) * 60 * 1000);
		
		strtokIndx = strtok(NULL,":");
		mF_CommandID[5] = strtokIndx;

		String postCmd = "{\"RemoteHostname\":\"" + hostname + "\",\"CommandID\":\"" + mF_CommandID[5].substring(0,36) + "\"}";

		client.publish(MQTT_TOPIC_ACK, postCmd.c_str());
		flgMfRunning[5] = true;
		ledcWrite(mF_PWMChannel[5], mF_PWMSpeed[5]);
		digitalWrite(mF_RelayPin[5], LOW);
	}

	if (strcmp(topic, "mixingFan/MF7") == 0 ){
		char* strtokIndx;
		
		strtokIndx = strtok(payloadStr,":");
		mF_PWMSpeed[6] = atoi(strtokIndx);
		
		strtokIndx = strtok(NULL,":");
		mF_OverSpeed[6] = atoi(strtokIndx);
		
		strtokIndx = strtok(NULL,":");
		mF_millisecondsUntilDone[6] = millis() + (atol(strtokIndx) * 60 * 1000);
		
		strtokIndx = strtok(NULL,":");
		mF_CommandID[6] = strtokIndx;

		String postCmd = "{\"RemoteHostname\":\"" + hostname + "\",\"CommandID\":\"" + mF_CommandID[6].substring(0,36) + "\"}";
    
		client.publish(MQTT_TOPIC_ACK, postCmd.c_str());
		flgMfRunning[6] = true;
		ledcWrite(mF_PWMChannel[6], mF_PWMSpeed[6]);
		digitalWrite(mF_RelayPin[6], LOW);
	}
}
...
void loop(void) {
	...
	mF_PWMSpeed[2] > 0 && flgMfRunning[2] ||
	mF_PWMSpeed[3] > 0 && flgMfRunning[3] ||
	mF_PWMSpeed[4] > 0 && flgMfRunning[4] ||
	mF_PWMSpeed[5] > 0 && flgMfRunning[5] ||
	mF_PWMSpeed[6] > 0 && flgMfRunning[6]) {
		updateTacho();
	}
	else
	{
